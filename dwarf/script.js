(function () {
  ...
  const AUTH_KEY = "styggmus-bingo-auth-v1:dwarf";
  const MODE_KEY = "styggmus-bingo-mode-v1:dwarf";
  const PLAYER_KEY = "styggmus-bingo-player-v1:dwarf";
  const BOARD_STORAGE_PREFIX = "styggmus-bingo-board-v2:dwarf";
  const BEERS_KEY = "styggmus-bingo-beers-v1:dwarf";
  ...
  const PARTY_TOPIC = "dwarf-bingo-forge-v1";
  const PARTY_KEY = "styggmus-bingo-party-v1:dwarf"; // localStorage: "off" disables
  ...
  const STATS_KEY = "styggmus-bingo-stats-v1:dwarf";
  ...
  const NIGHT_KEY = "styggmus-bingo-night-v1:dwarf";
  ...
  const promptGroups = [
    {
      ...
      prompts: [
        "Oski hämtar kaffe",
        "Oski svär högt vid skrivbordet",
        "Oski flexar ut tidigt",
        "Oski bokar ett möte som kunde varit ett mejl",
        "Oski klagar på wifi:et",
        "Oski tar en ”snabb” fråga som tar 20 minuter",
        "Oski glömmer att muta sig på Teams",
        "Oski skickar en gif i chatten",
      ],
    },
    {
      id: "huss",
      prompts: [
        "Huss nyser så hela kontoret hör",
        "Huss går på toa för femte gången",
        "Huss värmer fisk i mikron",
        "Huss suckar högt över sin inkorg",
        "Huss kommer försent till mötet",
        "Huss klagar på förkylningen",
        "Huss tar en extra lång fika",
        "Huss gäspar mitt i ett möte",
      ],
    },
    {
      id: "p1er",
      prompts: [
        "P1er visslar vid kaffemaskinen",
        "P1er längtar till after work",
        "P1er pratar om vädret",
        "P1er sträcker på sig och stönar",
        "P1er startar om datorn för att lösa allt",
        "P1er svarar på mejl mitt i ett möte",
        "P1er berättar samma historia igen",
        "P1er tar sista kaffet utan att brygga nytt",
      ],
    },
    {
      id: "andon",
      prompts: [
        "Andon utlöser en ”brådskande” deadline",
        "Andon kollar telefonen under mötet",
        "Andon bokar in ett extra standup-möte",
        "Andon glömmer namnet på en kollega",
        "Andon äter lunch vid skrivbordet",
        "Andon går hem utan att säga hej då",
        "Andon skickar ett mejl märkt ”VIKTIGT”",
        "Andon startar en diskussion om kontorstemperaturen",
      ],
    },
  ];
  ...
  function handleTypedEasterEgg(rawKey) {
    ...
    if (typedBuffer.includes("GULD")) {
      typedBuffer = "";
      triggerEasterEgg(
        "GULD!",
        "Ådern är funnen! Nästa runda måste presenteras med orimligt mycket självförtroende.",
        "ddko-mode"
      );
    }
  }
  ...
  function startEmbers() {
    ...
    const COLORS = ["rgba(255, 138, 40,", "rgba(255, 92, 24,", "rgba(255, 202, 74,"];
    let motes = [];
    let ratio = 1;
  ...
  function runConfetti(durationMs) {
    ...
    const W = window.innerWidth;
    const H = window.innerHeight;
    const colors = ["#ffca4a", "#ff7a2d", "#e0871f", "#ffdb9a", "#c98a3a"];
