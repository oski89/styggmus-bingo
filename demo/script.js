(function () {
  ...
  const AUTH_KEY = "styggmus-bingo-auth-v1:demo";
  const MODE_KEY = "styggmus-bingo-mode-v1:demo";
  const PLAYER_KEY = "styggmus-bingo-player-v1:demo";
  const BOARD_STORAGE_PREFIX = "styggmus-bingo-board-v2:demo";
  const BEERS_KEY = "styggmus-bingo-beers-v1:demo";
  ...
  const PARTY_TOPIC = "styggmus-bingo-demo-testroom-v1";
  const PARTY_KEY = "styggmus-bingo-party-v1:demo"; // localStorage: "off" disables
  ...
  const STATS_KEY = "styggmus-bingo-stats-v1:demo";
  ...
  const NIGHT_KEY = "styggmus-bingo-night-v1:demo";
  ...
  const promptGroups = [
    {
      id: "kasino",
      prompts: [
        "Någon säger ”en gång till” efter en förlust",
        "Någon påstår att de har ett vinnande system",
        "Diskussion om tur kontra skicklighet",
        "Någon skyller allt på otur",
        "Någon firar en liten vinst alldeles för stort",
        "Någon vill genast höja insatsen",
        "Någon räknar sina marker högt",
        "Någon bluffar och blir påkommen",
      ],
    },
    {
      id: "spel",
      prompts: [
        "Någon föreslår ett helt nytt spel",
        "Reglerna diskuteras i minst tio minuter",
        "Någon läser reglerna helt fel",
        "Någon vägrar erkänna att de förlorat",
        "Ett spel avbryts halvvägs",
        "Någon hittar på en egen husregel",
        "Två lag anklagar varandra för fusk",
        "Någon tar ett segervarv runt bordet",
      ],
    },
    {
      id: "dryck",
      prompts: [
        "Någon spiller sin dryck",
        "Någon hyllar snacksbordet",
        "Kylskåpet öppnas för femte gången",
        "Någon vill beställa mat",
        "Någon håller ett högtidligt skåltal",
        "Diskussion om vilken dryck som är bäst",
        "Någon glömmer vems glas som är vems",
        "Någon tar en ”allra sista” dryck",
      ],
    },
    {
      id: "musik",
      prompts: [
        "Spellistan byts mitt i en låt",
        "Någon höjer volymen lite för mycket",
        "Någon sjunger fel text med full inlevelse",
        "Alla kan refrängen utom en",
        "Någon vill spela ”bara en låt till”",
        "Någon dansar utan att bry sig om blickarna",
        "En gammal favoritlåt får jubel",
        "Någon plockar fram luftgitarren",
      ],
    },
    {
      id: "snack",
      prompts: [
        "Någon berättar samma historia igen",
        "Någon visar ett klipp för hela rummet",
        "Diskussion om vem som ska köra hem",
        "Någon letar febrilt efter sin telefon",
        "Två personer pratar i mun på varandra",
        "Någon somnar i soffan",
        "Någon vill ta en gruppselfie",
        "Diskussion om bästa filmen någonsin",
      ],
    },
  ];
  ...
  const players = [
    {
      id: "kasinomusen",
      label: "🐭 Kasinomusen",
      excludedGroup: "kasino",
    },
    {
      id: "tarningsmastaren",
      label: "🎲 Tärningsmästaren",
      excludedGroup: "spel",
    },
    {
      id: "olbaronen",
  ...
  function handleTypedEasterEgg(rawKey) {
    ...
    if (typedBuffer.includes("JACKPOT")) {
      typedBuffer = "";
      triggerEasterEgg(
        "JACKPOT!",
        "Alla klockor ringer! Nästa runda måste presenteras med orimligt mycket självförtroende.",
        "ddko-mode"
      );
    }
  }
  ...
  const REWARD_GAMES = {
    ...
    spy: { open: openSpykollen, overlay: spykollenOverlayEl, closeBtn: spyCloseBtn, label: "Spykollen", blurb: "Undvik 🤮 med 🛋️" },
