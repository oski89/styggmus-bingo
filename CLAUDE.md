...
bottom: `.bingo-topbar` (⋮ menu button + title only), `.stats` (Spelare and the
compact −/🍺/+ beer widget as its second item), then `.board-wrap`. There is no
Fält/Rader marked-count or bingo-line-count display — `updateStatsAndWinState`
still computes them internally (for win/reward detection) but doesn't render
